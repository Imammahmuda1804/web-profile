  ...
  useEffect(() => {
    getPortfolioData().then((d) => {
      if (d?.techStack && d.techStack.length > 0) {
        setStack(d.techStack);
      }
    });
  }, []);

  return (
    <section className="relative w-full bg-background text-foreground py-24 px-8">
      <div className="max-w-7xl mx-auto text-center">
        <AnimatedContent>
          <h2 className="text-3xl md:text-4xl font-bold mb-16">{t.tech.title}</h2>
        </AnimatedContent>
