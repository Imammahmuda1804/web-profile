  ...
  return (
    <div className="flex flex-col md:flex-row gap-y-2 gap-x-4 md:gap-x-8">
      ...
      <div className="md:w-44 text-left md:text-right shrink-0">
        <p className="font-semibold text-accent text-sm md:text-base">{date}</p>
        <p className="text-sm text-muted mt-0.5">{subtitle}</p>
      </div>

      {/* Timeline line & dot */}
      <div className="relative hidden md:block w-px bg-border">
        <div className="absolute w-3.5 h-3.5 rounded-full bg-accent ring-4 ring-background -left-[6px] top-1"></div>
      </div>

      {/* Content column */}
      <div className="grow pb-8 md:pb-12 border-l-2 border-border md:border-l-0 pl-4 md:pl-0">
        <h3 className="font-semibold text-foreground text-base md:text-lg leading-snug">{title}</h3>
        {description && (
